...
response = session.get('https://www.ub8.com/ajax/draw/ASHAREF2SSC/result?numOfDraw=3000')
data = response.json()
...
try:
    with engine.connect() as connection:
        ...
        select_max_query = text("SELECT MAX(nid) FROM base_data WHERE type = 2")
